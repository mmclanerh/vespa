...
protected:
    ...
    std::string getNodeList(std::vector<int> nodes);
...
std::string
BucketDBUpdaterTest::getNodeList(std::vector<int> nodes)
{
...
BucketDBUpdaterTest::testPendingClusterStateWithGroupDownAndNoHandover()
{
    ...
    std::string expSentNodes = "";
    if (_bucketSpaces.size() == 2u) {
        expSentNodes = "0,1,2,3,4,5";
    }
    CPPUNIT_ASSERT_EQUAL(
            expSentNodes,
            getSentNodes("distributor:6 storage:6",
